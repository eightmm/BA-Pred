...
def get_bond_feature(mol):
    ...
    Returns both edge directions (i->j and j->i) per bond, matching the original
    symmetric adjacency. The rotatable-bond flag keeps the original directional
    semantics: rf=1 only when the (src, dst) pair exactly matches a SMARTS match
    row. Output is permutation-equivariant downstream, so edge order is irrelevant.
    """
    rotate = get_indices(mol, "[!$(*#*)&!D1]-!@[!$(*#*)&!D1]")
    # Replicate the original `torch.tensor([a,b]) in rotate` semantics exactly:
    # torch __contains__ broadcasts -> True iff (a in rotate[:,0]) OR (b in rotate[:,1]).
    if len(rotate) != 0:
